...
func Test_wrap_map(t *testing.T) {
	...
	any := gosafejson.Wrap(map[string]string{"Field1": "hello"})
...
func Test_map_wrapper_any_get_all(t *testing.T) {
	should := require.New(t)
	any := gosafejson.Wrap(map[string][]int{"Field1": {1, 2}})
	should.Equal(`{"Field1":1}`, any.Get('*', 0).ToString())
	should.Contains(any.Keys(), "Field1")

	// map write to
	stream := gosafejson.NewStream(gosafejson.ConfigDefault, nil, 0)
	any.WriteTo(stream)
	// TODO cannot pass
